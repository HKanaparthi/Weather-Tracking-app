    ...
    function addLocationMarker(lat, lon, layerType) {
        ...
        // Prepare popup content based on layer type and weather data
        let popupContent = '';
        if (currentWeatherData && currentWeatherData.current) {
            ...
            switch(layerType) {
                case 'temp_new':
                    ...
                    <div class="map-popup">
                        ...
                        <div class="popup-temp">${Math.round(current.temperature)}°C</div>
                        <div class="popup-desc">${current.condition}</div>
                    </div>
                `;
                    break;
                case 'clouds_new':
                    // Approximate cloud coverage from condition if not available
                    let cloudCoverage = "N/A";
                    if (current.condition.includes('clear')) {
                        cloudCoverage = "0-10%";
                    } else if (current.condition.includes('few')) {
                        cloudCoverage = "10-25%";
                    } else if (current.condition.includes('scattered') || current.condition.includes('partly')) {
                        cloudCoverage = "25-50%";
                    } else if (current.condition.includes('broken')) {
                        cloudCoverage = "50-85%";
                    } else if (current.condition.includes('overcast') || current.condition.includes('cloudy')) {
                        cloudCoverage = "85-100%";
                    }

                    popupContent = `
                    <div class="map-popup">
                        <strong>${currentWeatherData.city}</strong>
                        <div class="popup-temp">Cloud Cover: ${cloudCoverage}</div>
                        <div class="popup-desc">${current.condition}</div>
                    </div>
                `;
                    break;
                case 'precipitation_new':
                    ...
                    <div class="map-popup">
                        <strong>${currentWeatherData.city}</strong>
                        <div class="popup-temp">Precipitation: ${current.precipAmount.toFixed(1)} mm</div>
                        <div class="popup-desc">Probability: ${current.precipProb}%</div>
                    </div>
                `;
                    break;
                case 'pressure_new':
                    ...
                    <div class="map-popup">
                        <strong>${currentWeatherData.city}</strong>
                        <div class="popup-temp">Pressure: ${current.pressure} hPa</div>
                        <div class="popup-desc">${current.condition}</div>
                    </div>
                `;
                    break;
                case 'wind_new':
                    ...
                    <div class="map-popup">
                        <strong>${currentWeatherData.city}</strong>
                        <div class="popup-temp">Wind: ${current.windSpeed.toFixed(1)} km/h</div>
                        <div class="popup-desc">Direction: ${current.windDirection}</div>
                    </div>
                `;
                    break;
                default:
            ...
            <div class="map-popup">
                ...
                <div class="popup-desc">Weather data not available</div>
            </div>
        `;
        }
    ...
    function updateDashboardWithWeatherData(data) {
        try {
            ...
            if (pm25Element) {
                pm25Element.textContent =pm25Element.textContent = data.current.pm25;
            }
    ...
    function handleDeviceSpecificBehavior() {
        const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;if (isTouchDevice) {
    ...
    function initializeApp() {
        ...
        // Initialize map layer selection event
        const mapLayerSelect = document.getElementById('mapLayer');
        if (mapLayerSelect) {
            mapLayerSelect.addEventListener('change', function() {
            ...
            if (preferredLayer) {
                mapLayerSelect.value = preferredLayer;
            }
        }
    }
